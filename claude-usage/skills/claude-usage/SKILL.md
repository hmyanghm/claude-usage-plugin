...
🔴 **출력 블록을 «있는 그대로» 먼저 보여준다.** 이 명령의 본체는 스크립트가 그린
   막대와 스파크라인이다 — 열이 맞춰져 있고 눈금이 팀 안에서 공통이라 **사람끼리
   높이를 눈으로 비교할 수 있는데**, 마크다운 표나 문장으로 다시 쓰면 그게 통째로
   사라진다. 등폭으로 뜨도록 코드 블록에 넣고, 한 줄도 바꾸거나 줄이지 말 것.

   - 팀이 여럿이면 **팀마다 형식을 달리하지 말 것.** 스크립트가 이미 같은 모양으로
     뽑아 놨다 — 한 사람짜리 팀이라고 문장으로 풀어쓰면 두 팀을 나란히 못 읽는다.
   - 사람이 하나뿐이어도, 스파크라인이 없어도 블록은 그대로 둔다.
   - `NO SIGNAL`·`(낡음)` 같은 표식도 지우지 말 것. 그게 「안 보이는 사람」을 설명한다.
...
- `7일 ▂▅▂▆▅█▃` — 하루하루 쓴 비용, 왼쪽이 6일 전 오른쪽이 오늘.
  **눈금이 그 팀 안에서 공통이라 사람끼리 높이를 비교해도 된다.** 마지막 줄에 `█` 가
  얼마인지 적혀 있다. 다만 `팀 합계` 줄만은 자체 눈금이라 개인 줄과 비교하면 안 된다.
- 비용은 **API 환산 추정치**다. 실제 청구액이 아니라 "얼마나 썼나" 의 크기다.

블록 **아래에** 판정을 붙인다. 블록을 풀어쓰는 게 아니라, 블록만 봐서는 바로 안
보이는 것을 한두 줄로 말한다. 사용자가 물은 게 있으면 그 답을 먼저:
...
- 물은 게 없으면 → 병목이 있는지 한 줄. 없으면 「지금은 다 여유 있다」로 끝낸다.
